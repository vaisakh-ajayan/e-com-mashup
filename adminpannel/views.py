    ...
    if request.user.is_authenticated:
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                ...
                return HttpResponse("your account deoesn't exists")
        else:
            login_form = LoginForm
            return render(request, 'adminpannel/login.html', {'form':form})
    else:
        login_form = LoginForm()
...
#user_passes_test decorator which performs a redirect when the callable returns False:
...
def addproduct(request):
    if request.method == 'POST':
        ...
        if product_form.is_valid():
            ...
            product_image = product_form.cleaned_data['product_image']
...
def editproduct(request, product_id):
    if request.method == 'POST':
        ...
        return render(request, 'adminpannel/editproduct.html', {
            'productform':product_form, 'current_image':product_instance.product_picture
            })
@user_passes_test(checksuperuser,login_url = reverse_lazy('login'))
...
def viewproduct(request, product_id):
    product_instance = Products.objects.get(id=product_id)
    return render(request, 'adminpannel/viewproduct.html', {'product_instance':product_instance})